...
def concat_lang_file():
    lang_contents = "##grflangid 0x01\n\n"
    lang_contents += "# AUTOGENERATED FILE, DO NOT MODIFY THIS FILE DIRECTLY, IT WILL BE OVERWRITTEN \n\n"
    lang_directory = Path("lang")
    lang_file = Path(lang_directory / "english.lng")

    plang_files = sorted(lang_directory.rglob("*.plng"))  

    for plang_file in plang_files:
        with open(str(plang_file), "r") as file:
            lang_contents += "# " + str(plang_file) + "\n"
            for line in file:
                lang_contents += line
            lang_contents += "\n\n"

    with open(lang_file, "w") as file_writer:
        for line in lang_contents:
...
def main(grf_name):
    ...
    error = compile_grf(grf_name, lang_directory)
    if error == -1:
        return -2
